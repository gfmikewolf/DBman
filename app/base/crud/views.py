...
# app
from config import Config# app/base/crud/views.py
# python
...
def view_record(table_name: str, pks: str) -> str:
    ...
    for key in basic_info:
        if key in rel_data['datajson_ref_names']:
            datajson_obj = DataJson.get_obj(basic_info[key])
            json_obj = json.loads(basic_info[key])
            for dj_key in datajson_obj.get_keys('Enum'): # type: ignore
                json_obj[dj_key] = getattr(datajson_obj,dj_key).name # type: ignore
            basic_info[key] = json_obj
        if key in model.get_col_keys('Enum'):
